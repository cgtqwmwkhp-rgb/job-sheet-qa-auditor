...
export function validateGoldSpec(spec: unknown): ValidationResult {
  ...
  if ("effectiveDate" in specObj && specObj.effectiveDate) {
    if (
      typeof specObj.effectiveDate !== "string" ||
      !isValidDateTime(specObj.effectiveDate)
    ) {
      errors.push({
        path: "$.effectiveDate",
  ...
  if ("expiryDate" in specObj && specObj.expiryDate) {
    if (
      typeof specObj.expiryDate !== "string" ||
      !isValidDateTime(specObj.expiryDate)
    ) {
      errors.push({
        path: "$.expiryDate",
 ...
 */
export function sanitizeGoldSpec(spec: unknown): {
  spec: unknown;
  ...
  return {
    spec: {
      ...
      description: specObj.description
        ? String(specObj.description).trim()
        : undefined,
