 ...
 */
const TRUNCATE_FIELDS = new Set(["prompt", "response", "error", "errorText"]);
  ...
  return {
    debug: (message: string, data?: Record<string, unknown>) => {
      if (process.env.LOG_LEVEL === "debug") {
        console.debug(
          formatEntry(createSafeLogEntry("debug", service, message, data))
        );
      }
    },
    info: (message: string, data?: Record<string, unknown>) => {
      console.log(
        formatEntry(createSafeLogEntry("info", service, message, data))
      );
    },
    warn: (message: string, data?: Record<string, unknown>) => {
      console.warn(
        formatEntry(createSafeLogEntry("warn", service, message, data))
      );
    },
    error: (message: string, data?: Record<string, unknown>) => {
      console.error(
        formatEntry(createSafeLogEntry("error", service, message, data))
      );
    },
  };
}
