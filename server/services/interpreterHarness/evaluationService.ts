...
function calculateMetrics(
  ...
  const precision =
    correct + falsePositives > 0 ? correct / (correct + falsePositives) : 0;
  const recall =
    correct + falseNegatives > 0 ? correct / (correct + falseNegatives) : 0;
  const f1Score =
    precision + recall > 0
...
function calculateSummary(run: EvaluationRun): EvaluationSummary {
  ...
  if (winner === "A" && confidenceLevel === "high") {
    ...
    recommendations.push(
      "Consider promoting candidate configuration (B) to production"
    );
  } else {
  ...
  if (latencyDelta > 100) {
    recommendations.push(
      `Config A is ${Math.round(latencyDelta)}ms slower - consider optimization`
    );
  } else if (latencyDelta < -100) {
    recommendations.push(
      `Config B is ${Math.round(Math.abs(latencyDelta))}ms slower - consider optimization`
    );
  }
...
function calculateFieldMetrics(results: TestCaseResult[]): FieldMetric[] {
  const fieldMap = new Map<
    string,
    { correct: number; total: number; confidenceSum: number }
  >();
