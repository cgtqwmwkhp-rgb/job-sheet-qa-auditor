 ...
 */
export const THRESHOLD_LEVELS: Record<
  ThresholdLevel,
  {
    globalMinConfidence: number;
...
export function createCalibrationProfile(
  ...
  for (const field of specJson.fields) {
    const isCritical =
      ALWAYS_CRITICAL_FIELDS.has(field.field) || field.required;
...
export function applyFieldCalibration(
  ...
  const accepted =
    adjustedConfidence >= calibration.minConfidence && extracted.extracted;
  const needsReview =
    !accepted && adjustedConfidence >= calibration.reviewThreshold;

  if (!extracted.extracted) {
    notes.push(`Field not extracted`);
  } else if (!accepted && needsReview) {
    notes.push(
      `Confidence ${adjustedConfidence} below min ${calibration.minConfidence}, needs review`
    );
  } else if (!accepted && !needsReview) {
    notes.push(
      `Confidence ${adjustedConfidence} below review threshold ${calibration.reviewThreshold}`
    );
  }
  ...
  for (const field of extractedFields) {
    ...
    if (!calibResult.accepted && calibResult.needsReview) {
      recommendations.push(
        `Review ${field.fieldId}: ${calibResult.notes.join(", ")}`
      );
    }
  }
...
export function runExtractionGuardrails(
  ...
  results.push({
    ...
    stopBehavior: g001Passed
      ? "CONTINUE"
      : getSeverityStopBehavior(g001Severity),
  ...
  results.push({
    ...
    stopBehavior: g002Passed
      ? "CONTINUE"
      : getSeverityStopBehavior(g002Severity),
    details:
      criticalWithLowConfidence.length > 0
  ...
  const conflicts = Array.from(valuesByField.entries()).filter(
    ([_, values]) => new Set(values).size > 1
  );
  ...
  results.push({
    ...
    stopBehavior: g003Passed
      ? "CONTINUE"
      : getSeverityStopBehavior(g003Severity),
    details:
      conflicts.length > 0
  ...
  const g004Passed =
    anomalyScore <= profile.anomalyThresholds.anomalyScoreThreshold;
  ...
  results.push({
    ...
    stopBehavior: g004Passed
      ? "CONTINUE"
      : getSeverityStopBehavior(g004Severity),
 ...
 */
export function evaluateGuardrailResults(results: GuardrailResult[]): {
  shouldStop: boolean;
  stopReason?: string;
...
function calculateAnomalyScore(
  ...
  const criticalFields = Array.from(profile.fieldCalibrations.values()).filter(
    c => c.isCritical
  );
