...
export function generateDailySummary(
  ...
  const reviewQueueCount = todayAudits.filter(
  ...
  >();
  for (const audit of todayAudits) {
    const existing = engineerMap.get(audit.engineerId) || {
  ...
  const topEngineers = Array.from(engineerMap.entries())
    ...
    .sort(
      (a, b) =>
        b.auditCount - a.auditCount || a.engineerId.localeCompare(b.engineerId)
    )
    .slice(0, 5);
...
export function generateMonthlyQualityPack(
  ...
  const customerMap = new Map<
    string,
    { name: string; valid: number; total: number; issues: Map<string, number> }
  >();
  ...
  const customerBreakdown = Array.from(customerMap.entries())
    ...
    .sort(
      (a, b) =>
        b.auditCount - a.auditCount || a.customerId.localeCompare(b.customerId)
    );

  // Asset type breakdown
  const assetTypeMap = new Map<
    string,
    { valid: number; total: number; issues: Map<string, number> }
  >();
  for (const audit of monthAudits) {
  ...
  const assetTypeBreakdown = Array.from(assetTypeMap.entries())
    ...
    .sort(
      (a, b) =>
        b.auditCount - a.auditCount || a.assetType.localeCompare(b.assetType)
    );
  ...
  if (averageValidRate < 0.8) {
    recommendations.push(
      "Overall valid rate is below 80%. Consider additional engineer training."
    );
  }

  const highAmbiguityTemplates = templateCockpit.templates.filter(
    t => t.metrics.ambiguityRate > 0.2
  );
  if (highAmbiguityTemplates.length > 0) {
    recommendations.push(
      `${highAmbiguityTemplates.length} templates have high ambiguity rates. Review selection fingerprints.`
    );
  }

  const lowConfidenceCustomers = customerBreakdown.filter(
    c => c.validRate < 0.7
  );
  if (lowConfidenceCustomers.length > 0) {
    recommendations.push(
      `${lowConfidenceCustomers.length} customers have valid rates below 70%. Consider targeted outreach.`
    );
  }
...
export async function executeMonthlyJob(
  ...
  try {
    ...
    const [audits, templateMetrics, customerData, assetTypeData] =
      await Promise.all([
        fetchAudits(),
        fetchTemplateMetrics(),
        fetchCustomerData(),
    ...
    const output = generateMonthlyQualityPack(
      audits,
      templateMetrics,
      customerData,
      assetTypeData
    ...
    logger.error("Monthly job failed", {
      error: error instanceof Error ? error.message : "Unknown",
    });
    return {
      jobName,
