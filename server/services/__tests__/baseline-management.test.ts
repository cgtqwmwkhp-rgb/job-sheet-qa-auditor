  ...
  describe("Content Hash Computation", () => {
    ...
    it("should produce same hash regardless of bySeverity key order", () => {
      const input1 = {
        version: "1.0.0",
        metrics: {
          bySeverity: canonicaliseBySeverity({
            S2: { passed: 1, total: 1 },
            S0: { passed: 1, total: 1 },
          }),
        },
    ...
    it("should detect severity threshold violation", () => {
      const violations = checkViolations(
        ...
        },
        {
          ...
          bySeverity: { S0: { minPassRate: 0.9, maxWorseCount: 0 } },
        }
      );
...
function validateCanonicalSeverity(
  ...
  if (legacyKeysFound.length > 0) {
    return (
      `Legacy severity keys found: ${legacyKeysFound.join(", ")}. ` +
      `Only canonical keys (${CANONICAL_SEVERITY_KEYS.join(", ")}) are allowed.`
    );
  }

  // Check for non-canonical keys
  const nonCanonicalKeys = keys.filter(
    k => !CANONICAL_SEVERITY_KEYS.includes(k)
  );

  if (nonCanonicalKeys.length > 0) {
    return (
      `Non-canonical severity keys found: ${nonCanonicalKeys.join(", ")}. ` +
      `Only canonical keys (${CANONICAL_SEVERITY_KEYS.join(", ")}) are allowed.`
    );
  }
...
function compareSeverities(
  ...
  const severities = sortSeverities(
    Array.from(new Set([...Object.keys(baseline), ...Object.keys(current)]))
  );
...
function calculateSeverityRate(data: {
  passed: number;
  total: number;
}): number {
...
function checkViolations(
  ...
  if (metrics.passRate < thresholds.overall.minPassRate * 100) {
    violations.push(
      `Overall pass rate ${metrics.passRate}% below threshold ${thresholds.overall.minPassRate * 100}%`
    );
  }
  ...
  Object.entries(metrics.bySeverity).forEach(([sev, data]) => {
    ...
    if (sevThreshold) {
      ...
      if (rate < sevThreshold.minPassRate * 100) {
        violations.push(
          `${sev} pass rate ${rate.toFixed(1)}% below threshold ${sevThreshold.minPassRate * 100}%`
        );
      }
    }
  });
