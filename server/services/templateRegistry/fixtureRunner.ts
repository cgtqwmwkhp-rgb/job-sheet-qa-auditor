...
function mockAnalyzeDocument(
  ...
  outcome: "pass" | "fail" | "review_queue";
  reasonCodes: string[];
  ...
  for (const field of specJson.fields) {
    ...
    const fieldNameParts = field.field
      .replace(/([A-Z])/g, " $1")
      .toLowerCase()
      .trim();
    ...
    const foundLabel = labelParts.some(
      part => part.length > 2 && text.includes(part)
    );
    const foundName = nameParts.some(
      part => part.length > 2 && text.includes(part)
    );
    ...
    if (foundLabel || foundName) {
      ...
      const hasCriticalRule = specJson.rules.some(
        r => r.field === field.field && r.severity === "critical"
      );
      if (hasCriticalRule) {
...
export function runFixtureMatrix(
  ...
  if (!pack) {
    throw new Error(
      `No fixture pack found for template version ${templateVersionId}`
    );
  }
  ...
  for (const testCase of pack.cases) {
    ...
    const {
      ...
      reasonCodes,
      fields: _fields,
    } = mockAnalyzeDocument(testCase.inputText, specJson, selectionConfig);
    ...
    if (outcome !== testCase.expectedOutcome) {
      errors.push(
        `Expected outcome '${testCase.expectedOutcome}' but got '${outcome}'`
      );
    }

    // Check reason codes if expected
    if (
      testCase.expectedReasonCodes &&
      testCase.expectedReasonCodes.length > 0
    ) {
      const missingCodes = testCase.expectedReasonCodes.filter(
        c => !reasonCodes.includes(c)
      );
      if (missingCodes.length > 0) {
        errors.push(
          `Missing expected reason codes: ${missingCodes.join(", ")}`
        );
        passed = false;
      }
    }
