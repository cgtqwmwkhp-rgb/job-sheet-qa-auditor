...
export function fuseFieldResults(
  ...
  if (!imageQaResult) {
    const outcome =
      ocrResult!.confidence >= FUSION_THRESHOLDS.minimumValidConfidence
        ? "VALID"
        : "LOW_CONFIDENCE";
    return {
      fieldId,
  ...
  if (!ocrResult) {
    const outcome =
      imageQaResult.confidence >= FUSION_THRESHOLDS.minimumValidConfidence
        ? "VALID"
        : "LOW_CONFIDENCE";
    return {
      fieldId,
  ...
  if (ocrPresent !== imageQaPresent) {
    ...
    if (
      ocrConf >= FUSION_THRESHOLDS.ocrHighConfidence &&
    ...
    if (ocrConf >= FUSION_THRESHOLDS.ocrHighConfidence) {
      return {
        ...
        fusionReason:
          "Trusting OCR (high confidence) over Image QA (low confidence)",
        cropReference,
      };
    }
    ...
    if (imageQaConf >= FUSION_THRESHOLDS.imageQaHighConfidence) {
      return {
        ...
        fusionReason:
          "Trusting Image QA (high confidence) over OCR (low confidence)",
        cropReference,
      };
    }
  ...
  if (
    ocrConf < FUSION_THRESHOLDS.ocrMediumConfidence ||
  ...
  const outcome =
    fusedConf >= FUSION_THRESHOLDS.minimumValidConfidence
      ? "VALID"
      : "LOW_CONFIDENCE";
  return {
    fieldId,
...
export function fuseAllFields(
  ...
  const hasLowConfidence = fields.some(
    f => f.fusedOutcome === "LOW_CONFIDENCE"
  );
