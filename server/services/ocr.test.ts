...
describe("Mistral OCR Service", () => {
  describe("API Key Validation", () => {
    it("should validate that MISTRAL_API_KEY is configured", async () => {
      ...
      if (!apiKeyExists) {
        console.warn(
          "MISTRAL_API_KEY not configured - skipping API validation test"
        );
        console.warn(
          "This is expected in CI environments without secrets access"
        );
      ...
      if (!result.valid) {
        console.warn(
          "Mistral API validation returned invalid. This may be a transient issue."
        );
