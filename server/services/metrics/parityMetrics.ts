...
export function recordIntegrityCheck(result: {
  ...
  metricsStore.integrity_last_check_epoch_seconds = Math.floor(
    Date.now() / 1000
  );
}
...
export function formatPrometheusMetrics(): string {
  ...
  const addMetric = (
    ...
    if (labels && Object.keys(labels).length > 0) {
      ...
      const validLabels = Object.entries(labels).filter(([k]) =>
        ALLOWED_LABELS.includes(k as (typeof ALLOWED_LABELS)[number])
      );
  ...
  addMetric(
    "parity_pass_rate",
    metrics.parity_pass_rate,
    "Current parity pass rate percentage",
    "gauge"
  );
  addMetric(
    "parity_total_fields",
    metrics.parity_total_fields,
    "Total number of validated fields",
    "gauge"
  );
  addMetric(
    "parity_passed_fields",
    metrics.parity_passed_fields,
    "Number of passed fields",
    "gauge"
  );
  addMetric(
    "parity_failed_fields",
    metrics.parity_failed_fields,
  ...
  // Counters (all numeric)
  addMetric(
    "parity_threshold_violations_total",
    metrics.parity_threshold_violations_total,
    "Total threshold violations",
    "counter"
  );
  addMetric(
    "parity_runs_total",
    metrics.parity_runs_total,
    "Total parity runs",
    "counter"
  );
  addMetric(
    "parity_failures_total",
    metrics.parity_failures_total,
    "Total parity failures",
    "counter"
  );
  addMetric(
    "integrity_mismatch_total",
    metrics.integrity_mismatch_total,
    "Total integrity mismatches",
    "counter"
  );
 ...
 */
export function validateMetricsSafety(metrics: ParityMetrics): {
  safe: boolean;
  issues: string[];
} {
  const issues: string[] = [];
