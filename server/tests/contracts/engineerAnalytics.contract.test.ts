...
describe("Engineer Analytics Contract Tests", () => {
  ...
  describe("Stable Ordering", () => {
    it("should maintain stable issue type ordering", () => {
      ...
      expect(scoreCard1.issuesByType.map(t => t.issueType)).toEqual(
        scoreCard2.issuesByType.map(t => t.issueType)
      );
    });
    ...
    it("should maintain stable recurring issue ordering", () => {
      ...
      expect(scoreCard1.topRecurringIssues.map(r => r.issueType)).toEqual(
        scoreCard2.topRecurringIssues.map(r => r.issueType)
      );
    });
    ...
    it("should maintain stable Fix Pack issue ordering", () => {
      ...
      expect(fixPack1.issues.map(i => `${i.issueType}:${i.fieldName}`)).toEqual(
        fixPack2.issues.map(i => `${i.issueType}:${i.fieldName}`)
      );
    });
  });
