...
describe("Drift Detection Contract Tests", () => {
  ...
  describe("Ambiguity Rate Detection", () => {
    it("should not alert when ambiguity rate is below warning threshold", () => {
      ...
      const alerts = detectAmbiguitySpike(
        data,
        baseline,
  ...
  describe("Token Collision Detection", () => {
    it("should not alert when collision rate is below threshold", () => {
      ...
      const alerts = detectTokenCollisions(
        data,
        baseline,
        DEFAULT_DRIFT_THRESHOLDS
      );
  ...
  describe("Override Spike Detection", () => {
    it("should not alert when override rate is normal", () => {
      ...
      const alerts = detectOverrideSpike(
        data,
        baseline,
        DEFAULT_DRIFT_THRESHOLDS
      );
  ...
  describe("Scan Quality Detection", () => {
    it("should not alert when quality is good", () => {
      ...
      const alerts = detectScanQualityDegradation(
        data,
        baseline,
        DEFAULT_DRIFT_THRESHOLDS
      );
  ...
  describe("Selection Accuracy Drift", () => {
    it("should not alert when accuracy is stable", () => {
      const alerts = detectSelectionAccuracyDrift(
        0.94,
        baseline,
        DEFAULT_DRIFT_THRESHOLDS
      );
  ...
  describe("Field Accuracy Drift", () => {
    it("should not alert when accuracy is stable", () => {
      const alerts = detectFieldAccuracyDrift(
        0.9,
        baseline,
        DEFAULT_DRIFT_THRESHOLDS
      );
  ...
  describe("Fusion Disagreement Detection", () => {
    it("should not alert when disagreement is low", () => {
      const alerts = detectFusionDisagreement(
        0.1,
        baseline,
        DEFAULT_DRIFT_THRESHOLDS
      );
  ...
  describe("Pass-2 Escalation Detection", () => {
    it("should not alert when escalation rate is normal", () => {
      const alerts = detectPass2Escalation(
        0.12,
        baseline,
        DEFAULT_DRIFT_THRESHOLDS
      );
  ...
  describe("EWMA / CUSUM (PR-18)", () => {
    it("should not alert on a flat defect-rate series", () => {
      const series = Array.from({ length: 10 }, (_, i) => ({
        t: `2024-01-${String(i + 1).padStart(2, "0")}`,
        rate: 0.12,
      }));
      const alerts = detectEwmaCusumDrift(series);
      expect(alerts).toHaveLength(0);
    });

    it("should alert when CUSUM detects an upward shift", () => {
      const series = [
        ...
        ...Array.from({ length: 8 }, (_, i) => ({
          t: `2024-01-${String(i + 6).padStart(2, "0")}`,
          rate: 0.4,
        })),
      ];
      const alerts = detectEwmaCusumDrift(series, {
        cusum: { k: 0.05, h: 0.12, minPoints: 4 },
        ewma: {
          lambda: 0.4,
          minPoints: 4,
          warningSigma: 1.5,
      ...
      expect(
        alerts.some(
          a => a.metric.startsWith("cusum_") || a.metric.startsWith("ewma_")
        )
      ).toBe(true);
    });
  });
