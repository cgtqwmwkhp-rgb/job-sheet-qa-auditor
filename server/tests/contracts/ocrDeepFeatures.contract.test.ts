...
const fixture = JSON.parse(
  readFileSync(join(__dirname, '../fixtures/mistral-ocr4-deep-response.json'), 'utf8')
);
...
describe('OCR Deep Features Contract (mock only)', () => {
  ...
  it('mock deep preset returns blocks, signatures, and confidence', async () => {
    ...
    expect(result.pages[0].confidenceScores?.averagePageConfidence).toBeGreaterThan(0);
  ...
  it('fixture parser + summarizeDeepFeatures stay PII-safe for reportJson', () => {
    ...
    const unsafe = checkLoggingSafety(summary as unknown as Record<string, unknown>);
