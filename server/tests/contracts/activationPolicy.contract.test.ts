...
const passingFixtures: FixtureCase[] = [
  {
    ...
    inputText:
      "Job Reference JOB-001 Asset ID ASSET-001 Date 2024-01-01 Engineer Sign Off Yes",
...
describe("Activation Policy - PR-K Contract Tests", () => {
  ...
  describe("Policy Check - Fixtures", () => {
    it("should fail when fixture pack is missing", () => {
      ...
      expect(
        result.violations.some(v => v.code === "MISSING_FIXTURE_PACK")
      ).toBe(true);
    });
  ...
  describe("Policy Check - ROIs", () => {
    it("should fail when critical ROIs are missing", () => {
      ...
      expect(result.allowed).toBe(false);
      expect(
        result.violations.some(v => v.code === "MISSING_CRITICAL_ROIS")
      ).toBe(true);
    });
    ...
    it("should allow specific ROIs to be missing via policy", () => {
      ...
      expect(result.warnings.some(w => w.code === "ALLOWED_MISSING_ROIS")).toBe(
        true
      );
    });
  });
    ...
    it("should fail when selection config is empty", () => {
      ...
      expect(
        result.violations.some(
          v =>
            v.code === "INSUFFICIENT_SELECTION_TOKENS" ||
            v.code === "SELECTION_CONFIG_EMPTY"
        )
      ).toBe(true);
    });
  });
  ...
  describe("Activation Report", () => {
    ...
    it("should include ROI presence details", () => {
      ...
      expect(report.roiPresence.criticalRoisMissing).toContain(
        "signatureBlock"
      );
    });
  ...
  describe("Deterministic Policy Checks", () => {
    it("should produce consistent results", () => {
      ...
      const result1 = runPolicyCheck(
        version.id,
        validSpecJson,
        validSelectionConfig,
        fullRoiConfig
      );
      const result2 = runPolicyCheck(
        version.id,
        validSpecJson,
        validSelectionConfig,
        fullRoiConfig
