...
describe("Stage 13d: Promotion Workflow Contract (ENFORCED)", () => {
  ...
  const ciGapPath = path.join(
    process.cwd(),
    "docs/patches/CI_GAP_PROMOTION_WORKFLOW.md"
  );
  ...
  const isOperationalised =
    workflowContent.includes("I_ACCEPT_PARITY_SKIP") &&
  ...
  describe("Gate Dependencies", () => {
    ...
    it("should require parity gate to pass (unless skip allowed)", () => {
      expect(workflowContent).toContain(
        "needs.parity-gate.result == 'success'"
      );
    });
  });
});
