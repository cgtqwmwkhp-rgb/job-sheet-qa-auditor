...
describe("Analyzer Fallback Contract", () => {
  ...
  describe("Graceful Degradation Behavior", () => {
    it("should log warning when using rule-based fallback", async () => {
      ...
      expect(analyzerContent).toContain(
        "LLM not configured - using rule-based analysis"
      );
    });
    ...
    it("should not throw error when LLM is not configured", async () => {
      ...
      const tryIndex = analyzerContent.indexOf(
        "try {",
        analyzerContent.indexOf("analyzeJobSheet")
      );
  ...
  describe("Rule-Based Analysis", () => {
    ...
    it("should NEVER return REVIEW_QUEUE from permissive rule-based fallback", async () => {
      const analyzerPath = path.resolve(
        __dirname,
        "../../services/analyzer.ts"
      );
      ...
      const functionMatch = analyzerContent.match(
        /function performRuleBasedAnalysis[\s\S]*?^}/m
      );
      if (functionMatch) {
        const functionBody = functionMatch[0];
        // Count REVIEW_QUEUE assignments in the function (should be 0)
        const reviewQueueAssignments = (
          functionBody.match(/overallResult\s*=\s*'REVIEW_QUEUE'/g) || []
        ).length;
        expect(reviewQueueAssignments).toBe(0);
      }

      // Also verify the design comment for permissive/dev path
      expect(analyzerContent).toContain("NEVER return REVIEW_QUEUE");
    });

    it("should return REVIEW_QUEUE for no-LLM in fail-closed environments", async () => {
      const analyzerPath = path.resolve(
        __dirname,
        "../../services/analyzer.ts"
      );
      ...
      expect(analyzerContent).toContain("isFailClosedEnvironment()");
      expect(analyzerContent).toContain('overallResult: "REVIEW_QUEUE"');
      expect(analyzerContent).toContain("AI judgment unavailable");
    });
