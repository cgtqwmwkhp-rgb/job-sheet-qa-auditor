...
describe("Deterministic Cache Service", () => {
  ...
  describe("Cache hit returns byte-identical outputs", () => {
    it("should return exact same data on cache hit", () => {
      ...
      // Retrieve from cache
      const { result: cachedResult, fromCache } = getCachedResult(
        fileContent,
        templateConfig
      );
    ...
    it("should miss cache for different file content", () => {
      ...
      const { fromCache } = getCachedResult(
  ...
  describe("Default configurations", () => {
    it("should have reasonable default parallelism caps", () => {
      ...
      expect(DEFAULT_PARALLELISM_CAPS.maxConcurrentRoiChecks).toBeGreaterThan(
        0
      );
      expect(
        DEFAULT_PARALLELISM_CAPS.maxConcurrentFieldExtractions
      ).toBeGreaterThan(0);
