...
function createMockTrace(
  ...
  return {
    ...
    candidates: (() => {
      const n = overrides.candidateCount ?? 2;
      const topSlug = overrides.templateSlug ?? "test-template";
      return Array.from({ length: n }, (_, i) => ({
        templateId: i + 1,
        templateSlug: i === 0 ? topSlug : `${topSlug}-runner-${i}`,
        versionId: i + 1,
        score:
          i === 0
            ? (overrides.topScore ?? 0.85)
            : (overrides.runnerUpScore ?? 0.45),
...
describe("Selection Analytics - PR-I Contract Tests", () => {
  ...
  describe("Overall Analytics", () => {
    ...
    it("should count auto-processed selections", () => {
      recordSelection(
        createMockTrace({ autoProcessingAllowed: true, selected: true })
      );
      recordSelection(
        createMockTrace({ autoProcessingAllowed: true, selected: true })
      );
      recordSelection(
        createMockTrace({ autoProcessingAllowed: false, selected: false })
      );
  ...
  describe("Token Collisions", () => {
    it("should identify tokens in multiple templates", () => {
      ...
      const maintenanceCollision = collisions.find(
        c => c.token === "maintenance"
      );
    ...
    it("should sort by collision count", () => {
      ...
      if (collisions.length > 1) {
        expect(collisions[0].collisionCount).toBeGreaterThanOrEqual(
          collisions[1].collisionCount
        );
      }
    });
  });
  ...
  describe("Template Analytics Summary", () => {
    it("should summarize per-template stats", () => {
      recordSelection(
        createMockTrace({ templateSlug: "template-a", confidenceBand: "HIGH" })
      );
      recordSelection(
        createMockTrace({ templateSlug: "template-a", confidenceBand: "HIGH" })
      );
    ...
    it("should calculate average score", () => {
      recordSelection(
        createMockTrace({ templateSlug: "template-x", topScore: 0.8 })
      );
      recordSelection(
        createMockTrace({ templateSlug: "template-x", topScore: 0.9 })
      );
  ...
  describe("Ambiguity Alert", () => {
    it("should alert when ambiguity exceeds threshold", () => {
      ...
      recordSelection(createMockTrace({ scoreDelta: 0.05, candidateCount: 2 }));
      recordSelection(createMockTrace({ scoreDelta: 0.05, candidateCount: 2 }));
      recordSelection(createMockTrace({ scoreDelta: 0.5, candidateCount: 2 }));
      recordSelection(createMockTrace({ scoreDelta: 0.5, candidateCount: 2 }));
  ...
  describe("Selection Records with Filtering", () => {
    it("should filter by template slug", () => {
      ...
      expect(result.records.every(r => r.templateSlug === "template-a")).toBe(
        true
      );
    });
    ...
    it("should return records in consistent order", () => {
      ...
      expect(result.records.map(r => r.jobSheetId)).toEqual(
        result2.records.map(r => r.jobSheetId)
      );
    });
  });
  ...
  describe("Deterministic Ordering", () => {
    it("should return consistent order for template summary", () => {
      ...
      expect(summary1.map(s => s.templateSlug)).toEqual(
        summary2.map(s => s.templateSlug)
      );
    });
    ...
    it("should return consistent order for token collisions", () => {
      ...
      expect(collisions1.map(c => c.token)).toEqual(
        collisions2.map(c => c.token)
      );
    });
  });
