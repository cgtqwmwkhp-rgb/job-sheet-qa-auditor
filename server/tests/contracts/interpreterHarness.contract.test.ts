...
describe("Interpreter A/B Harness Contract Tests", () => {
  ...
  describe("Evaluation Execution (Simulated)", () => {
    ...
    it("should produce deterministic results", async () => {
      ...
      expect(result1.resultsA.map(r => r.metrics)).toEqual(
        result2.resultsA.map(r => r.metrics)
      );
      expect(result1.resultsB.map(r => r.metrics)).toEqual(
        result2.resultsB.map(r => r.metrics)
      );
    });
  });
  ...
  describe("Aggregate Metrics", () => {
    ...
    it("should calculate latency percentiles", async () => {
      ...
      expect(metricsA?.p50LatencyMs).toBeLessThanOrEqual(
        metricsA?.p95LatencyMs || 0
      );
    });
