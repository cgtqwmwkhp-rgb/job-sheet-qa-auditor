...
describe("OCR Reconciliation Contract Tests", () => {
  ...
  describe("Field Reconciliation", () => {
    const sampleFields: ExtractedField[] = [
      ...
      },
      {
        ...
        confidence: 0.6,
        source: "primary",
        bbox: { x: 10, y: 20, width: 30, height: 5, pageNumber: 1 },
      },
      {
        ...
        confidence: 0,
        source: "primary",
        bbox: { x: 10, y: 40, width: 30, height: 5, pageNumber: 1 },
      },
    ];
    ...
    it("should only re-OCR fields with bbox", () => {
      ...
      const result = reconcileFields("doc-001", fieldsWithoutBbox, [
        "testField",
      ]);
    ...
    it("should produce deterministic output", () => {
      ...
      expect(result1.reconciledFields.map(f => f.fieldName)).toEqual(
        result2.reconciledFields.map(f => f.fieldName)
      );
  ...
  describe("Review Routing", () => {
    it("should route documents with missing required fields", () => {
      ...
      expect(routing.reasons.some(r => r.code === "MISSING_REQUIRED")).toBe(
        true
      );
    });
    ...
    it("should route documents with low confidence fields", () => {
      ...
      expect(routing.shouldRoute).toBe(true);
      expect(routing.reasons.some(r => r.code === "LOW_CONFIDENCE_FIELD")).toBe(
        true
      );
    });
  ...
  describe("Byte-Identical Output", () => {
    it("should produce identical output for identical input", () => {
      ...
      const normalizeArtifact = (s: string) =>
        s
          .replace(/"processedAt"\s*:\s*"[^"]+"/g, '"processedAt":"X"')
      ...
      const fields: ExtractedField[] = [
        {
          ...
          confidence: 0.5,
          source: "primary",
          bbox: { x: 0, y: 0, width: 10, height: 10, pageNumber: 1 },
        },
      ];
