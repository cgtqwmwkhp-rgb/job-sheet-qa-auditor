...
describe("Multi-Signal Template Recognition", () => {
  ...
  describe("Token Signal Extraction", () => {
    it("should extract token signal from document", () => {
      ...
      const result = extractTokenSignal(
        documentTokens,
        DEFAULT_SELECTION_CONFIG
      );
    ...
    it("should include matched tokens in evidence", () => {
      ...
      expect(
        result.evidence.matched.some((t: string) => t.includes("job"))
      ).toBe(true);
      expect(
        result.evidence.matched.some((t: string) => t.includes("sheet"))
      ).toBe(true);
    });
  });
  ...
  describe("Signal Combination", () => {
    ...
    it("should apply custom weights", () => {
      ...
      expect(customResult.combinedScore).toBeGreaterThan(
        defaultResult.combinedScore
      );
    });
  });
