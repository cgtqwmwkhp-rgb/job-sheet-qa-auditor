...
describe("Stage 13b: Deployment Promotion Gates (Hardened)", () => {
  ...
  describe("Artifact Ordering Determinism", () => {
    ...
    it("should produce stable ordering across multiple sorts", () => {
      ...
      const sorted1 = [...artifacts].sort((a, b) =>
        a.name.localeCompare(b.name)
      );
      const sorted2 = [...artifacts].sort((a, b) =>
        a.name.localeCompare(b.name)
      );
  ...
  describe("Bundle Hash Computation", () => {
    ...
    it("should NOT include timestamps in hash computation", () => {
      ...
      const hash1 = computeHashFromArtifacts(
        artifacts,
        "2025-01-01T00:00:00.000Z"
      );
      const hash2 = computeHashFromArtifacts(
        artifacts,
  ...
  describe("Gate Requirements", () => {
    ...
    const gateConfigs: Record<string, GateConfig> = {
      ci: {
        required: true,
        canSkip: false,
        skipRequiresApproval: false,
        skipAllowedForProduction: false,
      },
      policy: {
        required: true,
        canSkip: false,
  ...
  describe("Parity Skip Controls", () => {
    ...
    function validateParitySkip(config: {
      ...
      if (config.targetEnv === "production") {
        return {
          allowed: false,
          reason: "Parity skip is not allowed for production",
        };
      }
      ...
      if (config.acknowledgementText !== PARITY_SKIP_ACKNOWLEDGEMENT) {
        return {
          allowed: false,
          reason:
            "Parity skip requires acknowledgement text: " +
            PARITY_SKIP_ACKNOWLEDGEMENT,
        };
      }
    ...
    function simulateParityGate(config: {
      ...
      if (config.targetEnv === "production") {
        if (!config.parityPassed) {
          return {
            ...
            reason: "Parity failed - production promotion blocked",
          };
        }
      ...
      if (config.skipParity && config.skipAcknowledged) {
        return {
          ...
          reason: "Parity skipped with acknowledgement",
        };
      }

      return {
        promotionAllowed: false,
        reason: "Parity failed - staging promotion blocked",
      };
    }
  ...
  describe("Promotion Validation Rules", () => {
    function validatePromotion(config: {
      ...
      if (
        config.targetEnv === "staging" &&
        config.skipParity &&
        !config.skipAcknowledged
      ) {
  ...
  describe("Evidence Bundle Composition", () => {
    ...
    it("should format checksums correctly", () => {
      ...
      const checksums =
        artifacts.map(a => `${a.hash}  ${a.path}`).join("\n") + "\n";
    ...
    it("should include skip acknowledgement in manifest when applicable", () => {
      ...
      expect(manifestWithSkip.paritySkipAcknowledgement.acknowledged).toBe(
        true
      );
    });
  });
...
function computeHashFromArtifacts(
  artifacts: Array<{ hash: string }>,
  _timestamp: string
): string {
