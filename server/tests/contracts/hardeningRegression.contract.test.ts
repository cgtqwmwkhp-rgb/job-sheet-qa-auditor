...
describe("Hardening Regression Tests", () => {
  ...
  describe("Phase B: Versioned Selection Weights", () => {
    ...
    it("should have default signal weights defined", () => {
      expect(DEFAULT_SIGNAL_WEIGHTS.tokenWeight).toBe(0.4);
      expect(DEFAULT_SIGNAL_WEIGHTS.layoutWeight).toBe(0.2);
  ...
  describe("Phase C: Deterministic Stop Behavior", () => {
    it("should not stop when all guardrails pass", () => {
      ...
      const guardrailResults = runExtractionGuardrails(
        extractedFields,
        profile
      );
    ...
    it("should produce deterministic stop reasons", () => {
      ...
      const result1 = evaluateGuardrailResults(
        runExtractionGuardrails(extractedFields, profile)
      );
      const result2 = evaluateGuardrailResults(
        runExtractionGuardrails(extractedFields, profile)
      );
