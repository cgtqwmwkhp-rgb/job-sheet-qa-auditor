...
describe("Engineer Feedback Framework", () => {
  describe("generateEngineerScorecard", () => {
    ...
    it("should include top issues sorted by count", () => {
      const audits: AuditResult[] = [
        createMockAudit({
          outcome: "INVALID",
          reasonCodes: ["MISSING_FIELD", "LOW_CONFIDENCE"],
        }),
    ...
    it("should include template breakdown", () => {
      ...
      const templateA = scorecard.templateBreakdown.find(
        t => t.templateId === "template-001"
      );
    ...
    it("should support different aggregation periods", () => {
      const audits: AuditResult[] = [createMockAudit({ outcome: "VALID" })];

      const daily = generateEngineerScorecard(
        "eng-001",
        "John Smith",
        audits,
        "daily"
      );
      const weekly = generateEngineerScorecard(
        "eng-001",
        "John Smith",
        audits,
        "weekly"
      );
      const monthly = generateEngineerScorecard(
        "eng-001",
        "John Smith",
        audits,
        "monthly"
      ...
      const dailyDuration =
        new Date(daily.periodEnd).getTime() -
        new Date(daily.periodStart).getTime();
      const weeklyDuration =
        new Date(weekly.periodEnd).getTime() -
        new Date(weekly.periodStart).getTime();
  ...
  describe("generateEngineerFixPack", () => {
    ...
    it("should limit example audit IDs", () => {
      ...
      const missingFieldIssue = fixPack.issues.find(
        i => i.reasonCode === "MISSING_FIELD"
      );
  ...
  describe("generateTemplateQualityCockpit", () => {
    it("should generate cockpit with overall metrics", () => {
      ...
      const cockpit = generateTemplateQualityCockpit(
        audits,
        templateMetrics,
        "daily"
      );
    ...
    it("should include top issues across all templates", () => {
      ...
      const missingField = cockpit.topIssues.find(
        i => i.reasonCode === "MISSING_FIELD"
      );
  ...
  describe("Determinism", () => {
    it("should produce identical scorecards for identical inputs", () => {
      ...
      const scorecard1 = generateEngineerScorecard(
        "eng-001",
        "John Smith",
        audits,
        "daily"
      );
      const scorecard2 = generateEngineerScorecard(
        "eng-001",
        "John Smith",
        audits,
        "daily"
    ...
    it("should maintain stable ordering in cockpit templates", () => {
      ...
      const cockpit1 = generateTemplateQualityCockpit(
        audits,
        templateMetrics,
        "daily"
      );
      const cockpit2 = generateTemplateQualityCockpit(
        audits,
        templateMetrics,
        "daily"
      );
