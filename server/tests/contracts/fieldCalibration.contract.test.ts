...
describe("Field Calibration", () => {
  describe("Threshold Levels", () => {
    ...
    it("strict should have highest thresholds", () => {
      expect(THRESHOLD_LEVELS.strict.globalMinConfidence).toBeGreaterThan(
        THRESHOLD_LEVELS.standard.globalMinConfidence
      );
      expect(
        THRESHOLD_LEVELS.strict.criticalFieldMinConfidence
      ).toBeGreaterThan(THRESHOLD_LEVELS.standard.criticalFieldMinConfidence);
    });

    it("lenient should have lowest thresholds", () => {
      expect(THRESHOLD_LEVELS.lenient.globalMinConfidence).toBeLessThan(
        THRESHOLD_LEVELS.standard.globalMinConfidence
      );
      expect(THRESHOLD_LEVELS.lenient.criticalFieldMinConfidence).toBeLessThan(
        THRESHOLD_LEVELS.standard.criticalFieldMinConfidence
      );
    });
  });
  ...
  describe("Calibration Profile Creation", () => {
    ...
    it("should apply correct thresholds based on level", () => {
      const strictProfile = createCalibrationProfile(
        "test",
        DEFAULT_SPEC_JSON,
        "strict"
      );
      const lenientProfile = createCalibrationProfile(
        "test",
        DEFAULT_SPEC_JSON,
        "lenient"
      );

      expect(strictProfile.globalMinConfidence).toBeGreaterThan(
        lenientProfile.globalMinConfidence
      );
    });
    ...
    it("should include anomaly thresholds", () => {
      ...
      expect(
        profile.anomalyThresholds.maxMissingCriticalFields
    ...
    it("should set requireRoiForCriticalFields in strict mode", () => {
      ...
      const standardProfile = createCalibrationProfile(
        "test",
        DEFAULT_SPEC_JSON,
        "standard"
      );
  ...
  describe("Field Calibration Application", () => {
    ...
    it("should penalize failed validation pattern", () => {
      ...
      expect(result.notes.some(n => n.includes("validation pattern"))).toBe(
        true
      );
    });
  ...
  describe("Quality Assessment", () => {
    ...
    it("should detect missing critical fields", () => {
      ...
      expect(
        assessment.issues.some(
          i => i.type === "missing_field" && i.severity === "critical"
        )
      ).toBe(true);
    });
    ...
    it("should flag low confidence as issue", () => {
      ...
      expect(assessment.issues.some(i => i.type === "low_confidence")).toBe(
        true
      );
    });
  ...
  describe("Determinism", () => {
    it("should produce identical calibration profiles", () => {
      const profile1 = createCalibrationProfile(
        "test",
        DEFAULT_SPEC_JSON,
        "standard"
      );
      const profile2 = createCalibrationProfile(
        "test",
        DEFAULT_SPEC_JSON,
        "standard"
      );
      ...
      expect(profile1.fieldCalibrations.size).toBe(
        profile2.fieldCalibrations.size
      );
    });
    ...
    it("should produce identical quality assessments", () => {
      ...
      expect(assessment1.passedQualityGates).toBe(
        assessment2.passedQualityGates
      );
    });
