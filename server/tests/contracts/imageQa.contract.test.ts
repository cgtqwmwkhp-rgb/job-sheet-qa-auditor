...
`,
    ...
    it("should produce deterministic output", () => {
      ...
      expect(result1.documentQuality.overallScore).toBe(
        result2.documentQuality.overallScore
      );
      expect(result1.documentQuality.qualityGrade).toBe(
        result2.documentQuality.qualityGrade
      );
      expect(result1.summary.checkboxesFound).toBe(
        result2.summary.checkboxesFound
      );
    });
  });
  ...
  describe("Review Routing", () => {
    ...
    it("should set priority based on severity", () => {
      const result = analyzeDocumentQuality("doc-001", [
        {
          pageNumber: 1,
          markdown: "Good quality document with proper content.",
        },
      ]);
...
`;
      ...
      expect(result1.map(c => c.isChecked)).toEqual(
        result2.map(c => c.isChecked)
      );
    });
