...
describe("Image QA Fusion Service", () => {
  ...
  describe("fuseFieldResults", () => {
    describe("when both sources agree", () => {
      ...
      it("should return LOW_CONFIDENCE when both have low confidence", () => {
        ...
        expect(result.fusedConfidence).toBeLessThan(
          FUSION_THRESHOLDS.minimumValidConfidence
        );
      });
    });
    ...
    describe("ROI bbox and crop references", () => {
      ...
      it("should generate deterministic crop hash", () => {
        ...
        const result1 = fuseFieldResults(
          "signatureBlock",
          null,
          null,
          roiBbox,
        ...
        const result2 = fuseFieldResults(
          "signatureBlock",
          null,
          null,
          roiBbox,
        ...
        expect(result1.cropReference!.cropHash).toBe(
          result2.cropReference!.cropHash
        );
      });
    });
  });
  ...
  describe("fuseAllFields", () => {
    it("should process all Image QA fusion fields", () => {
      ...
      const roiBboxes = new Map<string, RoiBbox>([
        ...
        [
          "complianceTickboxes",
          { pageIndex: 0, x: 0.5, y: 0.4, width: 0.4, height: 0.3 },
        ],
      ]);

      const evidence = fuseAllFields(
        "doc-test",
        ocrResults,
        imageQaResults,
        roiBboxes
  ...
  describe("Determinism", () => {
    ...
    it("should produce deterministic overall outcome ordering", () => {
      ...
      const evidence1 = fuseAllFields(
        "doc-det-1",
        ocrResults,
        imageQaResults,
        roiBboxes
      );
      const evidence2 = fuseAllFields(
        "doc-det-1",
        ocrResults,
        imageQaResults,
        roiBboxes
      ...
      const normalize = (e: FusionEvidence) => ({
        ...
        fields: e.fields.map(f => ({
          ...f,
          cropReference: f.cropReference
            ? { ...f.cropReference, extractedAt: "X" }
            : undefined,
        })),
      });
