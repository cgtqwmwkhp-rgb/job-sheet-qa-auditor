...
const pilotPackPath = join(
  __dirname,
  "../../../data/pilot-templates/pilot-import-pack.json"
);
const pilotPack: BulkImportPack = JSON.parse(
  readFileSync(pilotPackPath, "utf-8")
);
...
describe("Pilot Templates - PR-G Contract Tests", () => {
  ...
  describe("Pilot Fixture Execution", () => {
    it("should run fixtures for maintenance template", () => {
      const result = importBulkPack(pilotPack, 1);
      const maintenanceResult = result.results.find(
        r => r.templateId === "maintenance-standard-v1"
      );
    ...
    it("should run fixtures for safety inspection template", () => {
      const result = importBulkPack(pilotPack, 1);
      const safetyResult = result.results.find(
        r => r.templateId === "safety-inspection-v1"
      );
    ...
    it("should run fixtures for installation certificate template", () => {
      const result = importBulkPack(pilotPack, 1);
      const installResult = result.results.find(
        r => r.templateId === "installation-cert-v1"
      );
  ...
  describe("Template Selection Fingerprints", () => {
    it("should have unique selection fingerprints", () => {
      ...
      for (let i = 0; i < fingerprints.length; i++) {
        for (let j = i + 1; j < fingerprints.length; j++) {
          const sameAll =
            JSON.stringify(fingerprints[i].requiredAll) ===
            JSON.stringify(fingerprints[j].requiredAll);
          const sameAny =
            JSON.stringify(fingerprints[i].requiredAny) ===
    ...
    it("should have form code regex for each template", () => {
      for (const template of pilotPack.templates) {
        ...
        expect(
          template.selectionConfigJson.formCodeRegex!.length
        ).toBeGreaterThan(0);
      }
    });
  });
