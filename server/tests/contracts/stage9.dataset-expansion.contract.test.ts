...
describe("Stage 9: Dataset Expansion Contract Tests", () => {
  ...
  describe("Dataset Size and Coverage", () => {
    ...
    it("should have ≥50 labelled documents (Wave-4 A3)", () => {
      expect(goldenDataset.documents.length).toBeGreaterThanOrEqual(50);
    });
    ...
    it("should have both pass and fail expected results", () => {
      const results = new Set(
        goldenDataset.documents.map(d => d.expectedResult)
      );
  ...
  describe("Reason Code Coverage", () => {
    it("passed fields should have no reason code (VALID is a status, not a reason code)", () => {
      ...
      const passedFieldsWithReasonCode = goldenDataset.documents.flatMap(d =>
        d.validatedFields.filter(
          f =>
            f.status === "passed" &&
            f.reasonCode !== undefined &&
            f.reasonCode !== null
        )
      );
  ...
  describe("Severity Coverage", () => {
    ...
    it("should have S0 failures in failed documents", () => {
      const failedDocs = goldenDataset.documents.filter(
        d => d.expectedResult === "fail"
      );
  ...
  describe("Edge Case Documents", () => {
    it("should have document with multiple failures", () => {
      const multiFailDoc = goldenDataset.documents.find(
        d => d.findings.length >= 3
      );
  ...
  describe("Threshold Configuration", () => {
    ...
    it("should have overall threshold defined", () => {
      ...
      expect(
        thresholdConfig.thresholds.overall.minPassRate
    ...
    it("should have PR subset ≥15 (Wave-4 A3)", () => {
      expect(thresholdConfig.ci.prSubsetDocIds.length).toBeGreaterThanOrEqual(
        15
      );
    });
