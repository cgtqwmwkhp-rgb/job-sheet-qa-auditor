  ...
  describe("Spec Resolver Contract", () => {
    ...
    it("resolves pack with deterministic rule ordering", () => {
      ...
      for (let i = 1; i < rules.length; i++) {
        ...
        expect(
          prev.localeCompare(curr, undefined, { numeric: true })
        ).toBeLessThan(0);
      }
    });
...
Total: $250.00
  ...
  describe("Validation Contract", () => {
    ...
    it("findings contain only failed rules", () => {
      ...
      const failedCount = result.validatedFields.filter(
        v => v.status === "failed"
      ).length;
    ...
    it("summary counts are accurate", () => {
      ...
      expect(
        summary.passedRules + summary.failedRules + summary.skippedRules
  ...
  describe("Review Queue Contract", () => {
    ...
    it("returns pending reviews in priority order", () => {
      ...
      for (let i = 1; i < pending.length; i++) {
        expect(pending[i - 1].priority).toBeLessThanOrEqual(
          pending[i].priority
        );
      }
    });
    ...
    it("provides accurate queue statistics", () => {
      ...
      expect(stats.total).toBe(
        stats.pending + stats.inProgress + stats.completed + stats.dismissed
      );
    });
  });
...
Total: $100.00
      ...
      const extractionArtifact = generateExtractionArtifact(
        extractionResult,
        "test-doc"
      );
      const validationArtifact = generateValidationArtifact(
        validationResult,
        "test-doc"
      );
      ...
      expect(validationResult.validatedFields.length).toBe(
    ...
    it("deterministic output across multiple runs", () => {
      ...
      expect(result1.validatedFields.map(v => v.ruleId)).toEqual(
        result2.validatedFields.map(v => v.ruleId)
      );

      // findings order should be identical
      expect(result1.findings.map(f => f.ruleId)).toEqual(
        result2.findings.map(f => f.ruleId)
      );
    });
  });
});
