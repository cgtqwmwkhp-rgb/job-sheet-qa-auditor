...
import { selectTemplateWithTrace } from "../../services/templateSelector";
...
describe("Activation Gates - PR-D Contract Tests", () => {
  ...
  describe("Activation Precondition Checks", () => {
    ...
    it("should fail when all critical fields are missing", () => {
      ...
      const missingFieldIssues = result.blockingIssues.filter(
        i => i.code === "MISSING_CRITICAL_FIELD"
      );
    ...
    it("should fail when selection config is empty", () => {
      ...
      expect(result.allowed).toBe(false);
      expect(
        result.blockingIssues.some(i => i.code === "SELECTION_CONFIG_EMPTY")
      ).toBe(true);
    });
    ...
    it("should fail when spec has no rules", () => {
      ...
      expect(result.allowed).toBe(false);
      expect(
        result.blockingIssues.some(i => i.code === "NO_VALIDATION_RULES")
      ).toBe(true);
    });
    ...
    it("should generate warnings for missing recommended fields", () => {
      ...
      const recommendedWarnings = result.warnings.filter(
        w => w.code === "MISSING_RECOMMENDED_FIELD"
      );
  ...
  describe("Selection Trace Always-On", () => {
    ...
    it("should include all candidates in trace with deterministic order", () => {
      ...
      const v2 = uploadTemplateVersion({
        ...
        // Distinct fingerprint (PR-16 collision governance) but still matches doc text
        selectionConfigJson: {
          requiredTokensAll: ["job", "document"],
      ...
      const result = selectTemplateWithTrace("Job sheet repair document", 999);
      ...
      for (let i = 1; i < result.trace.candidates.length; i++) {
        ...
        if (prev.score === curr.score) {
          expect(
            prev.templateSlug.localeCompare(curr.templateSlug)
