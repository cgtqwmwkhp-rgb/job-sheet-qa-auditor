...
describe("Gold Standard Spec Validation", () => {
  ...
  it("should reject spec with invalid version format", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some(e => e.message.includes("semantic versioning"))
    ).toBe(true);
  });
  ...
  it("should require pattern for regex type rules", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some(e => e.message.includes("Pattern is required"))
    ).toBe(true);
  });
...
describe("File Validation", () => {
  ...
  it("should validate file size limits", () => {
    ...
    const result = validateFile(largeBuffer, "application/pdf", {
...
describe("Rate Limiting", () => {
  ...
  it("should allow requests within limit", async () => {
    const result = await checkRateLimit("rate-test-1", {
      maxRequests: 5,
      windowMs: 60000,
    });
  ...
  it("should have predefined rate limits", () => {
    expect(RATE_LIMITS.upload.maxRequests).toBeLessThan(
      RATE_LIMITS.standard.maxRequests
    );
    expect(RATE_LIMITS.processing.maxRequests).toBeLessThan(
...
describe("Dead Letter Queue", () => {
  ...
  it("should retrieve failed job by ID", () => {
    const added = addToDeadLetterQueue(
      456,
      "analysis",
