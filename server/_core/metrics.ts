...
function formatAppMetrics(): string {
  ...
  lines.push(
    `app_info{git_sha="${escapeLabel(gitSha)}",version="${escapeLabel(platformVersion)}",node_version="${escapeLabel(nodeVersion)}"} 1`
  );
  ...
  // Auth redirect blocked counter
  lines.push(
    "# HELP auth_redirect_blocked_count Number of auth redirects blocked (returned 401 instead)"
  );
  ...
  lines.push(
    "# HELP pdf_proxy_range_requests_count Number of Range requests for PDF streaming"
  );
