/**
 * QUARANTINED — Stage 5 phantom review-queue API (PR-PLAT-STAGE5).
 *
 * In-memory store only. NOT mounted on appRouter. Production hold/review
 * uses live job-sheet + auditActions paths, not this scaffold.
 *
 * Do not mount until backed by real queue persistence.
 */
...
function sortReviewQueueItems(
  ...
  return [...items].sort((a, b) => {
    const priorityDiff =
      reasonPriority[a.reasonCode] - reasonPriority[b.reasonCode];
...
export const reviewQueueRouter = router({
  ...
  listOpen: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Get all open items
      let items = Array.from(reviewQueueStore.items.values()).filter(
  ...
  getReasonsForAudit: protectedProcedure
    ...
    .query(async ({ input }) => {
      const items = Array.from(reviewQueueStore.items.values()).filter(
        item => item.auditId === input.auditId
      );
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    const stats = {
      ...
      byReasonCode: {
        LOW_CONFIDENCE: items.filter(i => i.reasonCode === "LOW_CONFIDENCE")
          .length,
        UNREADABLE_FIELD: items.filter(i => i.reasonCode === "UNREADABLE_FIELD")
          .length,
