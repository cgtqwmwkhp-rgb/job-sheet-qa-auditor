/**
 * QUARANTINED — Stage 5 phantom pipeline API (PR-PLAT-STAGE5).
 *
 * In-memory runs + simulated PERSISTED when mock/no OCR key.
 * NOT mounted on appRouter. Production processing uses
 * `jobSheets.process` → orchestrateJobSheetProcessing (documentProcessor).
 *
 * Prefer quarantine over mounting this simulated pipeline as production.
 */
...
export const pipelineRouter = router({
  ...
  startRun: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!opts.forceRerun) {
        const existingRun = Array.from(pipelineStore.runs.values()).find(
          r => r.inputHash === inputHash && r.state !== "FAILED"
        );
  ...
  listRuns: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      let runs = Array.from(pipelineStore.runs.values()).sort(
        (a, b) => a.id - b.id
      );
