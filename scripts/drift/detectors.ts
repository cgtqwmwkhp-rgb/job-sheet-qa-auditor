...
export function detectAmbiguitySpike(
  ...
  if (severity) {
    alerts.push({
      ...
      suggestedAction:
        "Review template collision rules and consider adding disambiguation signals",
    });
  }
...
export function detectTokenCollisions(
  ...
  const currentRate =
    data.totalTokens > 0 ? data.collisions / data.totalTokens : 0;
  ...
  if (severity) {
    ...
    alerts.push({
      ...
      suggestedAction:
        "Review template token definitions for overlapping patterns",
    });
  }
...
export function detectOverrideSpike(
  ...
  const currentRate =
    data.totalDecisions > 0 ? data.overrides / data.totalDecisions : 0;
  ...
  if (severity) {
    alerts.push({
      ...
      deviationPercent: baselineRate > 0 ? (deviation / baselineRate) * 100 : 0,
      suggestedAction:
        "Investigate reason for increased manual overrides - may indicate extraction quality issues",
    });
  }
...
export function detectScanQualityDegradation(
  ...
  if (qualityDrop >= thresholds.scanQualityDrop.critical) {
    alerts.push({
      ...
      deviationPercent:
        baselineQuality > 0 ? (-qualityDrop / baselineQuality) * 100 : 0,
      suggestedAction:
        "Review OCR/image quality settings and check for scanner hardware issues",
    });
  ...
  const lowQualityRate =
    data.totalScans > 0 ? data.lowQualityScans / data.totalScans : 0;
  ...
  if (lowQualityRate > 0.2) {
    alerts.push({
      ...
      threshold: 0.2,
      baselineValue: 0.1,
      deviation: lowQualityRate - 0.1,
      deviationPercent: 100,
      suggestedAction:
        "Investigate scan quality issues across affected documents",
    });
  }
...
export function detectSelectionAccuracyDrift(
  ...
  if (drop >= thresholds.selectionAccuracyDrop.critical) {
    alerts.push({
      ...
      deviation: -drop,
      deviationPercent:
        baselineAccuracy > 0 ? (-drop / baselineAccuracy) * 100 : 0,
      suggestedAction:
        "Investigate template selection logic and recent changes",
    });
...
export function detectFusionDisagreement(
  ...
  if (severity) {
    alerts.push({
      ...
      deviationPercent:
        baselineDisagreement > 0
          ? ((disagreementRate - baselineDisagreement) / baselineDisagreement) *
            100
          : 0,
      suggestedAction:
        "Review fusion logic and investigate conflicting extraction sources",
    });
  }
...
export function detectPass2Escalation(
  ...
  const baselineRate = baseline?.pass2Rate ?? 0.1;
    ...
    alerts.push({
      ...
      deviationPercent:
        baselineRate > 0
          ...
          : 0,
      suggestedAction:
        "Investigate why more documents require interpreter escalation",
    });
  }
...
/**
 * Detect EWMA / CUSUM drift on a defect-rate (or similar) series (PR-18).
 */
export function detectEwmaCusumDrift(
  series: RatePoint[],
  options?: {
  ...
  const ewmaCfg = { ...DEFAULT_EWMA_CONFIG, ...options?.ewma };
  const cusumCfg = { ...DEFAULT_CUSUM_CONFIG, ...options?.cusum };
  const ewma = computeEwma(series, ewmaCfg);
  const cusum = computeCusum(series, cusumCfg);
  const alerts: DriftAlert[] = [];
  const metric = options?.metric ?? "defect_rate";

  if (ewma.severity !== "none") {
    alerts.push({
      id: generateAlertId("ewma"),
      category: "field_accuracy",
      severity: ewma.severity,
      status: "active",
      message: `EWMA drift on ${metric}: ${(ewma.lastEwma * 100).toFixed(1)}% (target ${(ewma.state.target * 100).toFixed(1)}%)`,
      detectedAt: new Date().toISOString(),
      metric: `ewma_${metric}`,
      ...
      baselineValue: ewma.state.target,
      deviation: ewma.lastEwma - ewma.state.target,
      deviationPercent:
        ewma.state.target > 0
          ? ((ewma.lastEwma - ewma.state.target) / ewma.state.target) * 100
          : 0,
      suggestedAction: "Investigate sustained rate shift in this series",
    });
  }

  if (cusum.severity !== "none" && cusum.direction === "increase") {
    alerts.push({
      id: generateAlertId("cusum"),
      category: "field_accuracy",
      severity: cusum.severity,
      status: "active",
      message: `CUSUM upward shift on ${metric}: S+=${cusum.lastSHigh.toFixed(3)}`,
      detectedAt: new Date().toISOString(),
      metric: `cusum_${metric}`,
      currentValue: cusum.lastSHigh,
      threshold: cusumCfg.h,
      baselineValue: cusum.state.target,
      deviation: cusum.lastSHigh,
      deviationPercent: 0,
      suggestedAction: "Review process changes driving the cumulative increase",
    });
  }
