...
export function calculateSelectionMetrics(
  results: EvalDocumentResult[]
): SelectionMetrics {
  const byTemplateId: Record<
    string,
  ...
  return {
    ...
    ambiguityRate:
      totalDocuments > 0 ? ambiguousSelections / totalDocuments : 0,
    byTemplateId,
    byAssetType,
  };
}
 ...
 */
export function calculateCriticalFieldMetrics(
  results: EvalDocumentResult[]
): CriticalFieldMetrics {
  ...
  return {
    ...
    bySeverity,
    byField,
    criticalOnlyAccuracy:
      criticalTotal > 0 ? criticalCorrect / criticalTotal : 0,
  };
}
...
export function calculateFusionMetrics(
  ...
  const byField: Record<
    string,
    { total: number; agreements: number; agreementRate: number }
  > = {};
 ...
 */
export function calculatePass2Metrics(
  results: EvalDocumentResult[]
): Pass2Metrics {
  const byTemplateId: Record<
    string,
    { total: number; triggered: number; rate: number }
  > = {};
  ...
  const metrics: Array<{ name: string; current: number; previous: number }> = [
    {
      ...
      current: current.selectionMetrics.accuracy,
      previous: previous.selectionMetrics.accuracy,
    },
    {
      name: "critical_field_accuracy",
      current: current.criticalFieldMetrics.accuracy,
      previous: previous.criticalFieldMetrics.accuracy,
    },
    {
      name: "fusion_agreement_rate",
      current: current.fusionMetrics.agreementRate,
      previous: previous.fusionMetrics.agreementRate,
    },
    {
      name: "pass2_rate",
      current: current.pass2Metrics.pass2Rate,
      previous: previous.pass2Metrics.pass2Rate,
    },
    {
      name: "overall_score",
...
export function generateRunId(): string {
  ...
  return `eval-${timestamp}-${counter}`;
}

/**
 * Sort document results deterministically
 */
export function sortDocumentResults(
  results: EvalDocumentResult[]
): EvalDocumentResult[] {
 ...
 */
export function sortFieldResults(
  results: EvalDocumentResult[]
): EvalDocumentResult[] {
  return results.map(result => ({
    ...result,
    fields: [...result.fields].sort((a, b) =>
      a.fieldId.localeCompare(b.fieldId)
    ),
    fusionResults: [...result.fusionResults].sort((a, b) =>
      a.fieldId.localeCompare(b.fieldId)
    ),
  }));
}
