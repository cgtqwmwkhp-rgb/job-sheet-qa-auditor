            ...
            >
              ...
              >
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="text-xl font-semibold text-foreground">{role.title}</CardTitle>
                  <CardDescription className="text-sm text-muted-foreground leading-relaxed">{role.description}</CardDescription>
                </CardHeader>
                <CardContent className="pt-2">
                  <ul className="space-y-2.5 mb-6">
                    {role.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-2.5 text-sm text-foreground/80">
                        <div className={`h-1.5 w-1.5 rounded-full ${role.primary ? 'bg-primary' : 'bg-[oklch(0.55_0.18_250)]'}`} />
                        {feature}
                      </li>
                    ))}
                  </ul>
