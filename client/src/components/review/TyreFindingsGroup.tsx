 ...
 * TyreFindingsGroup
 *
 * Clusters findings whose ruleId starts with "TYRE-C" under a
 * "Tyre compliance" collapsible group so auditors can review
 * tyre-related regulatory checks together.
 */
...
export interface TyreFindingsGroupProps {
  findings: Finding[];
...
export function isTyreComplianceFinding(finding: Finding): boolean {
  return !!finding.ruleId && finding.ruleId.startsWith("TYRE-C");
}

export function TyreFindingsGroup({
  findings,
  activeBoxId,
  onFindingClick,
  onReportIssue,
  ...
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50/30 overflow-hidden">
      ...
      >
        ...
        <CircleDot className="w-4 h-4 text-amber-700 shrink-0" />
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-1">
                  {finding.ruleId && (
                    <Badge
                      variant="outline"
                      className="font-mono text-[10px] px-1 bg-amber-50 text-amber-700 border-amber-300 shrink-0"
                    >
                      {finding.ruleId}
                    </Badge>
                  )}
